...
type AppState = Arc<Mutex<AppStatus>>;
type DbState = Arc<metadata::MetadataDb>;
...
async fn main() -> Result<()> {
    ...
    // This will serve the frontend from the `frontend/dist` directory.
    // You need to build the frontend first and place it there.
    let app = Router::new()
        .nest("/api", api_router)
...
async fn get_status(State((app_state, _db)): State<(AppState, DbState)>) -> Json<AppStatus> {
